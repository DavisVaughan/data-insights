...
After further reading into the guts of caret, I think my conclusions below are inaccurate, and the `train()$results` is fine to use. That being said, I would encourage you to look to Frank Harrell's [Comparison of Strategies](http://hbiostat.org/doc/simval.html) post that does a good job exploring various validation methods. One of the main takeaways is that the optimism bootstrap is fine when there are few predictors (or many predictors but with a _very_ large data set, probably ~10x the number of predictors at least based on a few simulations of my own). However, when there are a sizable number of predictors in a small-ish data set (say, 60 predictors with 500 observations), the optimism bootstrap can be less competitive than a repeated cross validation method.

There is clearly more room for improvement in understanding the exact weaknesses of these methods.

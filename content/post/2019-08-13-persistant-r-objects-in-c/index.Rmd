...
This is another entry in my series of R + C based posts (you can see a full list [here](https://blog.davisvaughan.com/)). This article focuses on a somewhat esoteric skill: constructing a global R object at the C level in a persistant way. By "persistant", I mean that this object will only be created once (at package load time), and will be reusable throughout the life of the R session. You'll be able to call it from other C files, and can even return the object to the R side. The other "trick" that will be used is a way to run arbitrary C code on R package load, using `.onLoad()` + `.Call()`. This is actually much more generic than what we will use it for in this article, so it is worth paying attention to in case you have other uses for it.
...
You can instead declare `int_one` as a global variable with a more permanant meaningful name, like `shared_int_one`, and use it without worrying about protection:

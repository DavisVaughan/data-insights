---
title: Persistant R Objects in C
...
This is another entry in my series of R + C based posts (you can see a full lists of posts [here](https://blog.davisvaughan.com/)). This article focuses on a somewhat esoteric skill: constructing a global R object at the C level in a persistant way. By "persistant", I mean that this object will only be created once (at package load time), and will be reusable throughout the life of the R session. You'll be able to call it from other C files, and can even return the object to the R side.
<span class="marginnote">
Most of these ideas are not my own. They are adaptations of ideas used by [Lionel Henry](https://twitter.com/_lionelhenry) in [`vctrs`](https://github.com/r-lib/vctrs) and [`rlang`](https://github.com/r-lib/rlang).
</span>

Why is this useful? I can think of two reasons. 

- The first is performance. You might have a simple R object (for instance, an integer vector holding `1`) that generally takes a small amount of time to create, but is generated and destroyed thousands of time across your C code base. To save a bit of time, you might want to make this a persistant, unchangeable, global variable.
...
You can instead declare `int_one` as a global variable with a more permanant meaningful name, like `shared_integer_one`, and use it without worrying about protection:

```{Rcpp}
SEXP result = PROTECT(Rf_allocVector(VECSXP, 1));

// can use `shared_integer_one` without creating a new one
SET_VECTOR_ELT(result, 0, shared_integer_one);
...
If you haven't read [Now You C Me](https://blog.davisvaughan.com/2019/03/02/now-you-c-me/), and you aren't too familiar with working on an R package with C code in it, you might want to go check out that post before continuing. It will teach you the basics of working with an R package containing C code.

The final product is an R package called [cshared](https://github.com/DavisVaughan/cshared). It contains one R function, `get_shared_objects()`. I'll discuss the bits and pieces of the package throughout the post, but that will be the ultimate reference for the end result.

### Setup

First, some setup. We'll leverage `{usethis}` and `{devtools}` to get our new package up and running. I'm assuming you are working in RStudio for this. The [Now You C Me](https://blog.davisvaughan.com/2019/03/02/now-you-c-me/) post describes these steps in much greater detail.
...
To call this from R, we need an `init.c` file that registers the C callable to the R side. We've done something like this in the other blog post, so create `init.c` and fill it with:
...
Over on the R side, we now need an R function that calls this `cshared_get_shared_objects` callable. Call `usethis::use_r("shared")` and fill the resulting R file with:
...
The next step is to replace our `empty_int` and `tidyverse` C variables with _shared_ global variables that were created at package load time. This will clean up our code a bit, and make the `cshared_get_shared_object()` function a bit easier to read. But accomplishing this requires some thought! What we want is a way to initialize some C SEXP objects _when the R package is loaded_. Generally, when we want to perform any action when a package is loaded we use the `.onLoad()` hook (see `?.onLoad` for more). To make it actually initialize our C variables, we will have to have `.onLoad()` use `.Call()` to call a C function that does the initialization.
...
- Register this initialization function as being callable from R like we did with `cshared_get_shared_objects()`.
- Call it from `.onLoad()`. 
...
If we `devtools::load_all()` now, we should have access to the `cshared_init_utils` routine object. This is what we need to `.Call()` from `.onLoad()`. I generally put my `.onLoad()` in `zzz.R`, as it is an auxillary function. It should be pretty simple:
